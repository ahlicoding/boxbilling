...
Create Dockerfile to run BoxBilling in your local environment, with these conditions:
Include module installation which needed by Boxbilling in your Dockerfile
Base image selection: Alpine (Better), Debian (Good), Ubuntu (Ok, no problem)
Serve boxbilling application to be accessible on port 8004 in your local environment.
Plus point: provide docker compose file which contain boxbilling + mysql, and database seeder (please let us know default credential you use for boxbilling)
Huge plus point: provide docker compose file which contain boxbilling + mysql + Landing Page from Step 1 (create separate Dockerfile for Landing Page), and database seeder (please let us know default credential you use for boxbilling)
Provide manual document (README.md) which includes step-by-step on how to run your Dockerfile (and/or docker compose file) in local environment

Put all those files for TEST - Step 2, in a single compressed archive (.zip, .tar.gz, etc), and the compressed file should be less than 500KB. Send all your answer for STEP 1 and STEP 2 to this email and cc to bayu.putra@hostinger.com
